...
\begin{enumerate}
    ...
    \begin{enumerate}
        \item[\textbf{RQ 1.1}] How accurately can a cross-lingual NLI baseline detect intrinsic hallucinations by evaluating the semantic entailment between English atomic claims and their Dutch source texts?
        \item[\textbf{RQ 1.2}] How effectively can the integration of the MusicBrainz external knowledge graph reduce the false positive rate of the baseline by validating extrinsic hallucinations?
        % \item[\textbf{RQ 1.3}] How does aggregating these claim-level evaluations through a strict Logical AND gate impact the final error detection rate at the sentence level?
    \end{enumerate}
\end{enumerate}
