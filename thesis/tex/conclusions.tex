...
Regarding threshold optimization (\textbf{RQ 1.2}), the confidence threshold ($\mathcal{T}$), our evaluations showed to lock the threshold at the optimal value of 70, showing that it only had a small impact on the metrics, mainly when the threshold was set to an extreme value ($\mathcal{T} \le 20$ or $\mathcal{T} \ge 90$). 
